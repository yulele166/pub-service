...
public class EopSetting {
	...
	public static String PRODUCT="b2b2c";
	//金额超过一定金额收取运费
	public static Double limitprice=69.0;	
	//收取的运费额度
	public static Double dlyprice=18.0;
	...
	//#解冻积分时长，单位是天
	public static Integer thawfreezepoint=15;
	//#未付款订单超时设置，单位是小时
	public static Integer paytimeout=24;
 ...
 //		InputStream in  =  FileUtil.getResourceAsStream("../../config/eop.properties"); 
		...
		if(VERSION==null) VERSION="b2c";
		//运费金额设置		
		limitprice = Double.valueOf(props.getProperty("limitprice"));
		if(limitprice==null) limitprice=69.0;
		dlyprice = Double.valueOf(props.getProperty("dlyprice"));
		if(dlyprice==null) dlyprice=18.0;
		
		CALLBACKURL = props.getProperty("callBackUrl");
		thawfreezepoint = Integer.parseInt(props.getProperty("thawfreezepoint"));
		paytimeout = Integer.parseInt(props.getProperty("paytimeout"));
